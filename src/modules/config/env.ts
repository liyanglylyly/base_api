...
export class Env {
  ...
  isProd() {
    return (
      this.run() === EnvironmentType.PRODUCTION ||
      this.run() === EnvironmentType.PROD
    );
  }
  ...
  isDev() {
    return (
      this.run() === EnvironmentType.DEVELOPMENT ||
      this.run() === EnvironmentType.DEV
    );
  }
